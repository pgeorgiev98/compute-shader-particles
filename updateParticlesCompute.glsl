...
layout(binding = 0, rgba32f) uniform image2D imageTexture;
...
void main() {
    ...
    vec4 destinationColor = imageLoad(imageTexture, ivec2(gl_GlobalInvocationID.xy));
    if (dist < ${minimumDistance}){
        ...
        imageAtomicAdd(colorTextureR, ivec2(v.x, v.y), int(destinationColor.r*255));
        imageAtomicAdd(colorTextureG, ivec2(v.x, v.y), int(destinationColor.g*255));
        imageAtomicAdd(colorTextureB, ivec2(v.x, v.y), int(destinationColor.b*255));
    }else{
